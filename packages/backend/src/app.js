...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    // Validate priority if provided, keep existing if not provided
    const validPriorities = ['P1', 'P2', 'P3'];
    const taskPriority = priority && validPriorities.includes(priority) ? priority : undefined;
    
    // Build update statement based on what's provided
    let updateFields = ['title = ?', 'description = ?', 'due_date = ?'];
    let values = [title, description || '', due_date || null];
    ...
    const stmt = db.prepare(`UPDATE tasks SET ${updateFields.join(', ')} WHERE id = ?`);
